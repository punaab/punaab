...
| `PROD_URL` | `https://punaab.vercel.app` |
...
| **HEARTBEAT STALE on dashboard** | Vercel **Hobby** only runs `vercel.json` cron **once per day**. For every 30 min, add GitHub secrets `CRON_SECRET` + `PROD_URL=https://punaab.vercel.app`. Or upgrade Vercel Pro. |
